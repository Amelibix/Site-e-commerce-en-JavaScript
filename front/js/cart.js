  ...
  if (validEmail(formEmail) && validFirstName(formFirstName) && validLastName(formLastName) && validAddress(formAddress) && validCity(formCity)) {
    ...
    fetch("http://localhost:3000/api/products/order", {
      ...
      .then(function (data) {
        ...
        let localStorageData = localStorage.getItem("test")
        let listProduct = JSON.parse(localStorageData)

        if (listProduct == null) {
          listProduct = [];
          listProduct.push(data.orderId);
        }

        localStorage.setItem("test", JSON.stringify(listProduct))
      })
